...
struct Rotation1dConst{T,Nsp,Nv}
    adv::Any
    t_perms::Any
    function Rotation1dConst(
        adv::Advection1d{T,Nsp,Nv,Nsum,timeopt},
    ) where {T,Nsp,Nv,Nsum,timeopt}
...
getperm(pc::Rotation1dConst, advd::Advection1dData) = pc.t_perms[_getcurrentindice(advd)]
getperm(pc::Rotation1dConst, curst::Int) = pc.t_perms[curst]


mutable struct Rotation1dVar{T,Nsp,Nv,Nsum} <: AbstractExtDataAdv1d{T,Nsum}
    pc::Rotation1dConst{T,Nsp,Nv}
    bufcur::Any
    function Rotation1dVar(pc::Rotation1dConst{T,Nsp,Nv}) where {T,Nsp,Nv}
...
getperm(pvar::Rotation1dVar, advd::Advection1dData) = getperm(pvar.pc, advd)
getperm(pvar::Rotation1dVar, curst::Int) = getperm(pvar.pc, curst)
...
function getrotationvar(adv::Advection1d)
    pc = Rotation1dConst(adv)
    return Rotation1dVar(pc)
end
...
"""
getalpha(pv::Rotation1dVar, self::Advection1dData, ind) = pv.bufcur[ind]
